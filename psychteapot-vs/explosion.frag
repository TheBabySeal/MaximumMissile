...
void main(void)
{
	...
	if((totalColor.r <= 0.05) && (totalColor.g <= 0.05) && (totalColor.b <= 0.05) && (type == 0))
	{
		discard;
	}
	else if((totalColor.r >= 0.95) && (totalColor.g >= 0.95) && (totalColor.b >= 0.95) && (type == 1))
	{
		discard;
	}
	outColor = totalColor; //  + calcDirLight(lightSourcesDirPosArr3) + calcDirLight(lightSourcesDirPosArr4)
